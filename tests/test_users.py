...
@pytest.mark.asyncio
async def test_list_user_organizations(client: AsyncClient, admin_headers, test_user, test_organization):
    ...
    if isinstance(data, list) and len(data) > 0:
        ...
        if isinstance(first_item, dict):
            assert any(org.get("organization_name") == "Test Organization" for org in data)
...
async def test_remove_user_from_organization(client: AsyncClient, admin_headers, test_user, db_session):
    ...
    from database.models import Organization, UserOrganizationRole
    ...
    response = await client.delete(
        f"/api/v1/users/{test_user.id}/organizations/{org.id}",
        headers=admin_headers,
    )
